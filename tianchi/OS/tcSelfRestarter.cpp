...
**
****************************************************************************/
#include "tcSelfRestarter.h"

#include <QCoreApplication>
#include <QDir>
#include <QProcess>

class TcSelfRestarter::Private
{
public:
...
{
    Q_ASSERT_X(!Private::instance, Q_FUNC_INFO, "Cannot create more than one TcSelfRestarter instance");
    Private::instance = this;
}

TcSelfRestarter::~TcSelfRestarter()
{
    Q_ASSERT_X(Private::instance == this, Q_FUNC_INFO, "Cannot create more than one TcSelfRestarter instance");
    delete d;
    Private::instance = 0;
}

void TcSelfRestarter::setRestartOnQuit(bool restart)
{
    Q_ASSERT_X(Private::instance, Q_FUNC_INFO, "TcSelfRestarter instance must be created in main()");
    if (Private::instance)
        Private::instance->d->restartOnQuit = restart;
}

bool TcSelfRestarter::restartOnQuit()
{
