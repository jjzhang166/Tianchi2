...
void TcDateEdit::calendarClicked(QDate _date)
{
    ...
    if ( count() == 0 )
    {
        addItem(_date.toString("yyyy-MM-dd"));
    }
    else
    {
        setItemData(0, QVariant::fromValue(_date.toString("yyyy-MM-dd")), Qt::DisplayRole);
    }
    setCurrentIndex(0);
    m_calendar->hide();
...
{
    clear();
    addItem(value.year() > 1900 ? value.toString("yyyy-MM-dd") : "");
    setCurrentIndex(0);
    //setEditText();
}
...
QString TcDateEdit::DateText(const QString& s)
{
    ...
    case 3: // m/d
        ...
        break;
    case 4: // mm/d or m/dd
        if ( dateText.mid(1, 1) == "/" || dateText.mid(1, 1) == "-" )
        {
            month = dateText.left(1).toInt();
            day   = dateText.right(2).toInt();
        }else
        if ( dateText.mid(2, 1) == "/" || dateText.mid(2, 1) == "-" )
        {
    ...
    {
        ret = QDate(year, month, day).toString("yyyy-MM-dd");
    }
    return ret;
}
