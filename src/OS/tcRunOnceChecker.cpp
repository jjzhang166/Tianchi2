...
class TcRunOnceChecker::Private
{
public:
...
TcRunOnceChecker::Private::Private(const QString &filename)
    : m_lockfile(filename)
    , m_hasLock(false)
{
...
TcRunOnceChecker::TcRunOnceChecker(const QString &filename)
    : d(new Private(filename))
{

}

TcRunOnceChecker::~TcRunOnceChecker()
{
    delete d;
}
...
bool TcRunOnceChecker::isRunning(Dependencies depends)
{
    bool running = false;
    switch (depends) {
        case Lockfile: {
        ...
        break;
        case ProcessList: {
            const QList<TcProcessInfo> allProcesses = TcRunInfo::runningProcesses();
